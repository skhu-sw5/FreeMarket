<template>
  <div @click="goToProduct" class="bg-white rounded-lg overflow-hidden shadow hover:shadow-md transition-all hover:translate-y-[-3px] transform duration-200 cursor-pointer relative">
    <div class="relative">
      <img 
      ...
      <div 
        v-if="imageError" 
        class="absolute inset-0 w-full h-full flex items-center justify-center text-gray-400 bg-gray-100"
      >
        <i class="fas fa-image text-4xl"></i>
      </div>
      <div 
        v-if="product.status === 'SOLD_OUT' || product.status === '판매완료'"
...
export default {
  ...
  emits: ['edit', 'delete', 'click', 'refresh', 'wishlist-toggle'],
  ...
  methods: {
    ...
    onImageError() {
      ...
      console.log('이미지 로딩 실패:', this.product.thumbnailUrl)
    },
